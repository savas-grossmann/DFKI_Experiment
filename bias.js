...
let expName = 'bias';  // from the Builder filename that created this script
let expInfo = {
...
async function experimentInit() {
  ...
  ranking_bar = new visual.TextStim({
    win: psychoJS.window,
    name: 'ranking_bar',
    text: 'Most likely < --- 1 --- 2 --- 3 --- 4 --- 5 --- 6 --- > Least likely',
    font: 'Arial',
    units: undefined, 
...
function Face_RankingRoutineBegin(snapshot) {
  return async function () {
    ...
    Face_RankingComponents.push(ranking_bar);
...
function Face_RankingRoutineEachFrame() {
  return async function () {
    ...
    // *ranking_bar* updates
    if (t >= 0.0 && ranking_bar.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      ranking_bar.tStart = t;  // (not accounting for frame time here)
      ranking_bar.frameNStart = frameN;  // exact frame index
      
      ranking_bar.setAutoDraw(true);
    }
    
    
    // if ranking_bar is active this frame...
    if (ranking_bar.status === PsychoJS.Status.STARTED) {
    }
